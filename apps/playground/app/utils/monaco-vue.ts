...
export function setupMonacoVue(monaco: typeof MT): void {
  ...
  // monaco-editor 0.55 types this whole namespace as `{ deprecated: true }` - the API
  // moved, but the runtime object is unchanged. Asserting to the package's own type
  // does not help (that type IS the stub), so describe just the parts used here.
