...
import { buildModuleUrls, buildHtml, consoleRelayScript, transpileTs } from '@playground/shared'
...
/**
 * Node mode can only run in ONE document per origin, and this claims that slot.
 *
 * almostnode's service worker keeps a single module-global `mainPort` and overwrites it
 * unconditionally on every `init` message, while its `registeredPorts` is a bare Set of
 * port numbers with no record of which client owns them. A service worker is shared across
 * every frame AND every tab of an origin, so a second node playground silently takes over
 * all preview traffic: the first one's preview starts being served by the second one's
 * server, or goes dead. Every exercise listens on 3001, so the ports cannot be told apart
 * either. Worse, when the browser recycles the worker it asks all clients to re-init and
 * ownership lands wherever the race lands — so the symptom is intermittent.
 *
 * None of that is fixable from here; it needs almostnode to key its routing by client. So
 * the second instance is told plainly instead of quietly breaking the first.
 *
 * Web Locks are origin-scoped, which matches the service worker's scope exactly, and the
 * lock is released automatically when the document goes away.
 */
const CONTAINER_LOCK = 'almostnode-container'
let lockHeld = false

function claimNodeSlot(): Promise<boolean> {
  // No Web Locks (older Safari): proceed rather than block the only working mode.
  if (!import.meta.client || !navigator.locks?.request) return Promise.resolve(true)

  return new Promise<boolean>(resolve => {
    navigator.locks.request(CONTAINER_LOCK, { ifAvailable: true }, lock => {
      resolve(lock !== null)
      // Returning a promise that never settles is how a lock is held for the lifetime of
      // the document; releasing it here would hand the slot straight back.
...
// The relay posts to window.parent, which for a node-mode preview is this app - the
// preview iframe is a direct child. No origin argument: it derives its own.
const CONSOLE_SCRIPT_HTML = consoleRelayScript()
...
export const useAlmostNode = (files: Record<string, string>) => {
  ...
  async function boot(): Promise<void> {
    ...
    // Tracks HELD, not merely checked. Once held, asking again would be denied by our own
    // lock; but after a denial, Restart has to be able to retry - that is the whole point
    // of telling the student to close the other tab and press it.
    if (!lockHeld) {
      if (!await claimNodeSlot()) {
        isBooting.value = false
        bootStatus.value = 'Already open elsewhere'
        pushLog(
          'The Node playground is already running in another tab or frame. '
          + 'Only one can run at a time, because they would share one service worker '
          + 'and serve each other’s pages. Close the other one and press Restart.',
          LogType.ERROR,
        )
        return
      }
