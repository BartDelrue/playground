...
// and this one is only ever picked for vue.
defineProps<{ mode?: string; displayMode?: DisplayMode }>()
...
const dependencyVersion = ref<Record<string, string>>(dependencyVersionsFromImportMap(builtinImportMap.value.imports))
...
// NB: unrelated to the workspace's own `typescript` devDependency, which is on 5.9 so
// vue-tsc can run (TS 6 dropped the typescript/lib/tsc export it needs). This string
// only selects what the in-browser Volar worker downloads. Do not "align" them.
